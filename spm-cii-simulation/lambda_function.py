...
def lambda_handler(event, context):
    ...
    if plan == "Voyage":
        ...
        tmp_res_simulation_speed = select.get_simulation_speed(imo)

        if tmp_res_simulation_speed and tmp_res_simulation_speed[0]["year"]["S"] == now_year:
            res_simulation = select.get_simulation_speed(imo)
    
    simulation_execution_flg = False

    if res_simulation and res_simulation[0]["flag"]["S"] == "1":
        # Simulationテーブルが取得できたかつ、使用flagが1の場合、Simulation処理を実施
        ...
        if plan == "Voyage":
            ...
            # Leg内総FOCを算出
            leg_total_foc       = ballast_foc + laden_foc
            port_total_foc      = auxiliary_equipment / 24 * port_time
            leg_total_FOC_speed = leg_total_foc + port_total_foc
    ...
    if res_simulation:
        ...
        if plan == "Voyage":
            ...
            if res_foc_formulas: 
                ...
                port_total_foc = auxiliary_equipment / 24 * port_time
                # Leg内総FOCを算出
                leg_total_FOC_speed = round(float(ballast_foc + laden_foc + port_total_foc), 1)
        ...
        # Simulation実施なし　かつ　Speedプランの場合、「time_to_endofyear」のみセット
        if plan == "Speed":
            # Time to End of Year算出（年末 - 現在）
            year_end = datetime(dt_now.year, 12, 31, 23, 59, 59)
            time_to_end_of_year = calc_time_diff(dt_now, year_end)
