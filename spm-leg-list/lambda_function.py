...
def lambda_handler(event, context):
    ...
    for imo in imo_list:
        ...
        EUA_Year_to_Date    = year_total_data["ytd_eua"] if float(year_total_data["ytd_eua"]) != 0 else ""
        EUA_End_of_Year     = year_total_data["eoy_eua"] if float(year_total_data["eoy_eua"]) != 0 else ""
        CB_Year_to_Date     = year_total_data["ytd_cb"] if float(year_total_data["ytd_cb"]) != 0 else ""
        CB_End_of_Year      = year_total_data["eoy_cb"] if float(year_total_data["eoy_cb"]) != 0 else ""
