...
def lambda_handler(event, context):
    ...
    for imo in imo_list:
        ...
        if res_simulation_voyage and res_simulation_voyage[0]["flag"]["S"] == "1":
            ...
            for i in range(len(res_simulation_voyage)):
                ...
                if departure_time.year > dt_now.year:
                    print(f"来年以降なので処理スキップ(departure_time:{(departure_time)} arrival_time:{(arrival_time)})→simulation_leg:{(res_simulation_voyage[i])}")
                    continue
                ...
                calculated_sailing_time = calculate_function.calc_time_diff(start_time, end_time)
                ...
                print(f"処理するレグ(start_time:{(start_time)} end_time:{(end_time)}) → leg_total_time:{(leg_sailing_time)} leg_actual_time:{(calculated_sailing_time)}")
        ...
        print(f"imo:{(imo)} tmp_eof_cii_value:{(tmp_eof_cii_value)}")
